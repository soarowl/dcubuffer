module dcubuffer.util;

import std.range : OutputRange;
import std.system : Endian, endian;
import std.traits : isArray;

import dcubuffer.buffer : ForeachType, Buffer, canSwapEndianness;
 ...
 */
class Typed(T, B:
        Buffer = Buffer) : B, OutputRange!T if (canSwapEndianness!T)
{
...
/// ditto
template Typed(T, B:
        Buffer = Buffer) if (isArray!T && canSwapEndianness!(ForeachType!T))
{
    alias Typed = Typed!(ForeachType!T, B);
}
...
pure @safe unittest
{
    ...
    buffer.write(2);
    version (BigEndian)
        assert(buffer.data!ubyte == [0, 0, 0, 1, 0, 0, 0, 2]);
    version (LittleEndian)
        assert(buffer.data!ubyte == [1, 0, 0, 0, 2, 0, 0, 0]);
...
pure @trusted unittest
{
    ...
    buffer.put(Test(1, 2));
    version (BigEndian)
        assert(buffer.data!ubyte == [0, 0, 0, 1, 0, 2, 0, 0]);
    version (LittleEndian)
        assert(buffer.data!ubyte == [1, 0, 0, 0, 2, 0, 0, 0]);
    ...
    version (BigEndian)
        assert(buffer.data!ubyte == [0, 0, 2, 0, 1, 0, 0, 0]);
    version (LittleEndian)
        assert(buffer.data!ubyte == [0, 0, 0, 2, 0, 0, 0, 1]);
